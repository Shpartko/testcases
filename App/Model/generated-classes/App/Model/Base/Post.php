...
abstract class Post implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Model\\Map\\PostTableMap';
    ...
    /**
     * Initializes internal state of App\Model\Base\Post object.
     */
     ...
     * @return $this|\App\Model\Post The current object (for fluent API support)
     */
    ...
    public function setUsername($v)
    {
        ...
        if ($this->username !== $v) {
            $this->username = $v;
            $this->modifiedColumns[PostTableMap::COL_USERNAME] = true;
        }
    ...
    public function setTopicId($v)
    {
        ...
        if ($this->topic_id !== $v) {
            $this->topic_id = $v;
            $this->modifiedColumns[PostTableMap::COL_TOPIC_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PostTableMap::translateFieldName('Username', TableMap::TYPE_PHPNAME, $indexType)];
            $this->username = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PostTableMap::translateFieldName('TopicId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : PostTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Model\\Post'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PostTableMap::COL_USERNAME)) {
            $modifiedColumns[':p' . $index++]  = 'username';
        }
        if ($this->isColumnModified(PostTableMap::COL_TOPIC_ID)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Model\Post The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PostTableMap::COL_USERNAME)) {
            $criteria->add(PostTableMap::COL_USERNAME, $this->username);
        }
        if ($this->isColumnModified(PostTableMap::COL_TOPIC_ID)) {
            $criteria->add(PostTableMap::COL_TOPIC_ID, $this->topic_id);
        }
        if ($this->isColumnModified(PostTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Model\Post (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Model\Post Clone of current object.
     * @throws PropelException
     */
